...
**Team:** Manny, Anthony · both full-time students, part-time work
**Coding starts:** Saturday **September 5, 2026**
**Demo:** Friday **October 9, 2026**
**Code freeze:** Friday **October 2** — after this, only bug fixes and polish
**Fallback if the Sep 25 checkpoint slips:** Friday October 16

Weeks run Saturday to Friday, matching the Sep 5 start. Every date below was
recomputed against the real 2026 calendar — an earlier version of this file had
weekday labels that did not match their dates (it called Sep 5 a Friday; it is
a Saturday).
...
|---|---|---|
| **Fri Sep 11** | ~~Agent auth ADR written.~~ Done Sep 5. Draft endpoint shape agreed. Client proxy working. | Everything slides a week. The Oct 9 demo becomes Oct 16. |
...
|---|---|---|---|---|---|
| ~~Review open PRs~~ **DONE** | Anthony approved #21 and #23; all four PRs merged Sep 4 | Anthony | Sep 4 | 3 | — |
...
| Wire relevance into entries | `scoreEntry` + `ScoredTeammate` type + three call sites | Anthony | Sep 24 | 5 | Merge the collector, Reason generator PR |
| Relevance on page load | Recompute relevance on `GET /timeline` | Anthony | Sep 26 | 2 | Wire relevance into entries |
| Relevance wiring tests | Tests for `scoreEntry`, mutation-verified | Anthony | Sep 26 | 2 | Wire relevance into entries |
| VPN signature | `VPN_LOOPBACK` signature + engine + tests | Anthony | Sep 24 | 7 | Environment scanner |
| Wire room details | Wire room details + timeline REST | Manny | Sep 24 | 5 | Wire rooms list, Bring in room details page |
...
| Agent HTTP client | HTTP client — login (via AgentKey), create-or-reuse incident, draft, publish | Anthony | Oct 1 | 3.5 | Agent credentials build, Incident reuse by system, AI delivery PR |
...
| End-to-end run | End-to-end: agent → server → both browsers | Both | Oct 3 | 4 | Agent run loop, Relevant-to-you section |
| Update contracts doc | `contracts.md` — agent section, draft route, confirm route, new socket payload | Anthony | Oct 3 | 1.5 | End-to-end run |
| UI polish pass | UI/UX polish pass | Manny | Oct 8 | 6 | End-to-end run |
...
**Owner:** Anthony · **Due:** Wed Sep 9 · **Est:** 3h · **Blocked by:** —
...
**Done when:** ~~#20 and #24 both show APPROVED.~~ **Done Sep 4** — Anthony approved #21 and #23, and all of #19 through #24 are merged.

### Send account email
**Owner:** Anthony · **Due:** Tue Sep 8 · **Est:** 5 min · **Blocked by:** —
...
### Merge the queue — DONE
**Owner:** Manny · **Due:** Thu Sep 10 · **Est:** 1.5h · **Blocked by:** Review open PRs
...
**Done when:** ~~`main` has `server/src/ai`, `server/src/relevance`, `.mailmap`, and the full suite passes.~~ **Done Sep 4** — all present on `main`, 94 tests passing, `tsc` clean.

### Add path map keys — DONE
**Owner:** Manny · **Due:** Wed Sep 9 · **Est:** 30 min · **Blocked by:** — · **Status:** done, commit 086602b on PR #20
...
**Owner:** Anthony · **Due:** Wed Sep 9 · **Est:** 30 min · **Blocked by:** —
...
### AI delivery PR
**Owner:** Anthony · **Due:** Sat Sep 12 · **Est:** 2h · **Blocked by:** Merge the queue, Settle draft endpoint shape
...
### Reason generator PR
**Owner:** Anthony · **Due:** Sat Sep 12 · **Est:** 2h · **Blocked by:** Merge the queue
...
**Owner:** Anthony · **Due:** Sat Sep 12 · **Est:** 30 min · **Blocked by:** —
...
### Build the collector
**Owner:** Manny · **Due:** Thu Sep 17 · **Est:** 8h · **Blocked by:** Merge the queue
...
### Collector tests
**Owner:** Manny · **Due:** Sat Sep 19 · **Est:** 3h · **Blocked by:** Build the collector
...
### Merge the collector
**Owner:** Manny · **Due:** Sat Sep 19 · **Est:** 1h · **Blocked by:** Collector tests, Anthony's review
...
### Wire sign-in
**Owner:** Manny · **Due:** Tue Sep 15 · **Est:** 4.5h · **Blocked by:** Client proxy setup, Merge the queue
...
### Wire room details
**Owner:** Manny · **Due:** Thu Sep 24 · **Est:** 5h · **Blocked by:** Wire rooms list, Bring in room details page
...
### Wire live updates
**Owner:** Manny · **Due:** Sat Sep 26 · **Est:** 4h · **Blocked by:** Wire room details
...
### Confirm button
**Owner:** Manny · **Due:** Sat Sep 26 · **Est:** 1.5h · **Blocked by:** Wire live updates, AI delivery PR
...
**Description:** The actual implementation of ADR 0016. `agent_credentials` migration (mirrors `refresh_tokens`: `user_id` FK, `key_hash`, `label`, `revoked_at`). `POST /auth/agent-keys` — authenticated, generates with `signRefreshToken()`'s pattern, hashes with `hashRefreshToken()`'s pattern, returns the raw key once. `requireAgentKey` middleware, parallel to `requireAuth`, populating `req.user` in the same shape so no existing handler changes. **Blocks Agent HTTP client** — Anthony cannot authenticate for real until this exists.
...
**Owner:** Anthony · **Due:** Sat Sep 12 · **Est:** 1.5h · **Blocked by:** —
...
### Environment scanner
**Owner:** Anthony · **Due:** Sat Sep 19 · **Est:** 9h · **Blocked by:** Agent scaffold and contract
...
### VPN signature
**Owner:** Anthony · **Due:** Thu Sep 24 · **Est:** 7h · **Blocked by:** Environment scanner
...
**Owner:** Anthony · **Due:** Wed Sep 30 · **Est:** 5h · **Blocked by:** VPN signature
...
### Agent run loop
**Owner:** Anthony · **Due:** Fri Oct 2 · **Est:** 2.5h · **Blocked by:** Private card and gate, Agent HTTP client
...
**Owner:** Anthony · **Due:** Thu Sep 24 · **Est:** 5h · **Blocked by:** Merge the collector, Reason generator PR
...
**Owner:** Anthony · **Due:** Sat Sep 26 · **Est:** 2h · **Blocked by:** Wire relevance into entries
...
**Owner:** Manny · **Due:** Thu Oct 1 · **Est:** 5h · **Blocked by:** Wire relevance into entries, Relevance on page load, Wire live updates
...
**Owner:** Manny · **Due:** Fri Oct 2 · **Est:** 2h · **Blocked by:** Wire rooms list
...
### Demo seed data
**Owner:** Anthony · **Due:** Thu Sep 17 · **Est:** 2h · **Blocked by:** Merge the queue
...
### CI on pull requests
**Owner:** Anthony · **Due:** Wed Sep 16 · **Est:** 2h · **Blocked by:** —
...
### End-to-end run
**Owner:** Both · **Due:** Sat Oct 3 — **CODE FREEZE** · **Est:** 4h · **Blocked by:** Agent run loop, Relevant-to-you section
...
**Owner:** Anthony · **Due:** Sat Oct 3 · **Est:** 1.5h · **Blocked by:** End-to-end run
...
### Dogfood week
**Owner:** Both · **Due:** Wed Oct 7 · **Est:** 3h across the week · **Blocked by:** End-to-end run
...
### Fix dogfood bugs
**Owner:** Both · **Due:** Fri Oct 9 · **Est:** 3h · **Blocked by:** Dogfood week
...
### UI polish pass
**Owner:** Manny · **Due:** Thu Oct 8 · **Est:** 6h · **Blocked by:** End-to-end run
