...
describe('socket auth middleware', () => {
  ...
  function connect(auth?: Record<string, unknown>): ClientSocket {
    const client = ioc(`http://localhost:${port}`, { reconnection: false, ...(auth ? { auth } : {}) });
    clients.push(client);
    return client;
  }
  ...
  it('accepts a valid token and stamps identity onto socket.data', () => {
    ...
    return new Promise<void>((resolve, reject) => {
      ...
      client.on('connect_error', (err) => reject(new Error(`unexpected connect_error: ${err.message}`)));
    });
  });
  ...
  it('rejects a connection with no token', () => {
    return new Promise<void>((resolve, reject) => {
      ...
      client.on('connect', () => reject(new Error('connected without a token — middleware did not block it')));
