...
// The half-life is imported, not declared here, so all three tuned numbers sit in one file and
// -> nobody adjusts one without seeing the others. constants.ts says why they are guesses.
import { RELEVANCE_HALF_LIFE_DAYS } from './constants.js';

// Exported because score.ts needs the same conversion for its frequency window.
