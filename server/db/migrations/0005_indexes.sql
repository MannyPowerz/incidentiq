...
-- Postgres errors immediately rather than warning and continuing. CONCURRENTLY avoids
-- locking a table under live write traffic — this schema has none yet, so the
-- restriction costs nothing real today. Revisit only if this ever runs against a table
